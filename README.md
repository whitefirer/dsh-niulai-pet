...
循环喊进行期间，对着麦克风喊一声**「牛来」**，循环立即停止——停下时妈妈还会
回一句「牛来！」。识别是**零模型**的浏览器内模板匹配（MFCC + 子序列 DTW），
模板就是妈妈的回应音本身（`assets/reply.mp3`，电影里妈妈喊的那声「牛来」），
不下载任何模型，音频不出浏览器。
...
- **授权流程**：打开开关时会真实试取一次麦克风（浏览器弹原生授权框），
  ...
  「未授权 / 已授权 / 不可用」。
- 判别力离线标定见 `test/voice-matcher.mts`：正样本（本体+变调/变速/加噪）
  最高 ≈0.45 < 阈值 0.57 < 负样本（「妈妈」喊声/静音/白噪）最低 ≈0.71。
  注意宁紧勿松——宠物自己喊的「妈妈」被误认成「牛来」会让循环自己停掉。
  真人嗓音的召回率受麦克风与距离影响，需真机微调。
