...
识别是**零模型**的浏览器内模板匹配（MFCC + 子序列 DTW），模板是同一句
「牛来！」的两条录音（`assets/reply_match.mp3` 长切版 + `reply_ref.mp3`
参考版，取最小分），不下载任何模型，音频不出浏览器。
...
- 判别力离线标定见 `test/voice-matcher.mts`：正样本（本体+变调/变速/窄带）
  最高 ≈0.41 < 阈值 0.52 < 负样本（「妈妈」喊声/静音/白噪）最低 ≈0.62，
  且需连续 2 次过阈才判命中（防抖）。注意宁紧勿松——宠物自己喊的「妈妈」
